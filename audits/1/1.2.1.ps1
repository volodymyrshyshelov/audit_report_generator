...
try {
    # Get groups with visibility 'Public'
    $publicGroups = Get-MgGroup -All | Where-Object { $_.Visibility -eq "Public" }

    if (-not $publicGroups) {
        $output = "SUCCESS: No public groups found. All groups are private."
    } else {
        $output = "WARNING: Found public groups:`n"
        $output += ($publicGroups | Select-Object DisplayName, Visibility | Format-Table -AutoSize | Out-String)
    }
...
try {
    ...
    Write-Host "ERROR: Evaluation failed: $_`n" -ForegroundColor Red
}
